/*
 * File name: mac_sender.c
 * Authors: Guillaume Raetz, Sylvan Arnold
 * Date: 05/05/2024
 *
 * Description:
    * This file contains the mac sender thread. It is used to send data to the physic layer.
    * It also sends token frames and new token frames.
    * It also sends error messages to the LCD when we can't send a message to a specific station.
    * It also sends token list messages to the LCD when the token list has changed.
    * It also sends back data frames when they are not acknowledged.
    * It also sends data frames to the physic layer.
 */
